...
def set_balance(update, context):
    ...
    if result is not None:
        if len(args) == 1:
            ...
            # Het saldo bijwerken in de database
            cursor.execute('UPDATE children SET balance = ? WHERE chat_id = ?', (new_balance, chat_id))
            conn.commit()

            context.bot.send_message(chat_id=chat_id, text="Zakgeldsaldo is bijgewerkt!")
        else:
            context.bot.send_message(chat_id=chat_id, text="Ongeldige configuratie. Gebruik: /setbalance <saldo>")
    else:
        context.bot.send_message(chat_id=chat_id, text="Je bent nog niet geregistreerd. Stuur /start om te beginnen.")
